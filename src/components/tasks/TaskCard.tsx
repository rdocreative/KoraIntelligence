...
export const TaskCard = ({ task, isAwaitingTime, onUpdateTime, defaultPeriodTime }: TaskCardProps) => {
  ...
  const getGradientStyle = (period?: string) => {
    switch (period) {
      case 'Morning':
        return 'linear-gradient(180deg, rgba(249,115,22,0.13) 0%, rgba(249,115,22,0.02) 100%)';
      case 'Afternoon':
        return 'linear-gradient(180deg, rgba(34,197,94,0.13) 0%, rgba(34,197,94,0.02) 100%)';
      case 'Evening':
        return 'linear-gradient(180deg, rgba(129,140,248,0.13) 0%, rgba(129,140,248,0.02) 100%)';
      case 'Dawn':
        return 'linear-gradient(180deg, rgba(56,189,248,0.13) 0%, rgba(56,189,248,0.02) 100%)';
      default:
  ...
  const style = {
    ...
    touchAction: 'none',
    background: getGradientStyle(task.period)
  };
  ...
  return (
    <div
      ...
      data-draggable="true"
      className={cn(
        "relative group/card flex flex-col p-4 rounded-[20px] transition-all cursor-grab active:cursor-grabbing overflow-hidden min-h-[90px] border-none",
        isDragging && "z-50 shadow-2xl bg-white/[0.08]"
      )}
    >
