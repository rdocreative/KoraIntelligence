  ...
  return (
    <div className="space-y-10 outline-none animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out">
      ...
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 pt-4">
           <div className="space-y-2 max-w-3xl">
              ...
              <div>
                ...
                <p className="mt-4 text-base md:text-lg text-neutral-500/80 font-medium max-w-xl leading-relaxed">
           ...
           <div className="flex flex-col items-end gap-3 w-full lg:w-auto">
              <div className="flex justify-between w-full lg:w-72 text-xs font-bold uppercase tracking-[0.15em] text-neutral-600">
                  <span>Conclusão Anual</span>
                  <span className="text-white text-shadow-sm">{annualData.progress}%</span>
              </div>
              <div className="w-full lg:w-72 h-1.5 bg-white/5 rounded-full relative overflow-hidden">
                 <div 
                    className="absolute top-0 left-0 h-full bg-gradient-to-r from-red-800 to-red-500 rounded-full transition-all duration-1000 ease-out"
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* HERO CARD - TRANSPARENTE */}
        <Card className="lg:col-span-2 bg-transparent border-white/10 shadow-none relative overflow-hidden group hover:border-red-500/30 transition-all duration-500">
          <div className="absolute top-0 right-0 p-12 opacity-[0.02] group-hover:opacity-[0.04] transition-opacity duration-500 scale-150 pointer-events-none">
            <Target className="w-64 h-64" />
          </div>

          <CardHeader className="relative z-10 pb-2">
            <div className="flex items-center gap-2 mb-2">
              <span className="w-1.5 h-1.5 rounded-full bg-red-500 shadow-[0_0_10px_red]" />
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-6">
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-neutral-600 tracking-widest">Progresso</span>
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-neutral-600 tracking-widest">Tarefas</span>
                    <span className="text-2xl font-bold text-white">{activeWeeks[0].tasks.filter((t: any) => t.completed).length} <span className="text-base text-neutral-700 font-medium">/ {activeWeeks[0].tasks.length}</span></span>
                  </div>
                </div>
                <Button onClick={onNavigateToWeekly} variant="outline" className="bg-white/5 hover:bg-white/10 text-white border-white/10 rounded-lg">
                  Ver Detalhes <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              </div>
            ) : (
              <div className="py-8 flex flex-col items-start gap-4">
                <p className="text-neutral-600 text-lg">Nenhum foco definido.</p>
        ...
        {/* MONTH CARD - TRANSPARENTE */}
        <Card className="bg-transparent border-white/10 shadow-none hover:border-white/20 transition-all duration-300 flex flex-col justify-between">
          <CardHeader>
            <CardTitle className="text-xs font-bold text-neutral-600 uppercase tracking-[0.2em]">Mês Atual</CardTitle>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-end justify-between">
              <div className="text-5xl font-black text-white tracking-tighter">
                ...
                <span className="text-lg text-neutral-700 font-medium ml-1">/ {currentMonth.goals.length}</span>
              </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <Card key={i} className={`bg-transparent border-white/5 shadow-none transition-all duration-300 ${area.border} group`}>
            <CardContent className="pt-6 flex flex-col items-center text-center gap-3">
              <div className={`p-3 rounded-xl bg-white/5 group-hover:bg-white/10 transition-colors`}>
                <area.icon className={`w-5 h-5 ${area.color}`} />
              </div>
              <div>
                <span className="block font-bold text-xs text-white uppercase tracking-wider">{area.title}</span>
                <span className="text-[10px] text-neutral-600 font-bold mt-1 block tracking-tight">{area.items.filter(i => i.completed).length}/{area.items.length} CONCLUÍDOS</span>
              </div>
            </CardContent>
          </Card>
        ))}
