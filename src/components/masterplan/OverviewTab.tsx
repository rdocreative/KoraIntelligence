  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <CardContent className="p-8 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-6">
            <div className="space-y-2">
              ...
              <h2 className="text-3xl font-bold text-white tracking-tight">{annualData.objective || "Defina seu objetivo principal"}</h2>
            </div>
          ...
          <div className="h-4 bg-neutral-800 rounded-full overflow-hidden mb-6 border border-white/5">
            ...
            >
              <div className="absolute inset-0 bg-white/10" style={{ backgroundImage: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.2), transparent)', transform: 'skewX(-20deg)' }}></div>
            </div>
          </div>
              ...
              return (
                <div key={area.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <Progress value={area.percentage} className="h-1.5 bg-neutral-800" />
                  </div>
                </div>
              );
            })}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {currentWeek ? (
              <div className="flex flex-col mt-1">
                ...
                <div className="flex items-end gap-2 mt-1">
                  ...
                  <span className="text-sm text-neutral-500 mb-1">tarefas feitas</span>
                </div>
              </div>
            ) : (
               <span className="text-sm text-neutral-500 mt-2 block">Nenhuma semana ativa.</span>
            )}
          </CardHeader>
          <CardContent className="flex-1 flex flex-col justify-end">
            {currentWeek && <Progress value={weekProgress} className="h-2 bg-neutral-800 mb-4" />}
            <Button 
              variant="outline" 
        ...
        <Card className={`${cardBaseStyle} border-primary/20 flex flex-col`}>
          <CardHeader className="pb-2">
            <div>
            ...
            <div className="space-y-3 mt-2 flex-1">
              ...
              {(!currentWeek || currentWeek.tasks.filter((t:any) => !t.completed).length === 0) && (
                <div className="flex flex-col items-center justify-center h-full text-center py-2">
                  ...
                  <p className="text-xs text-neutral-500">Tudo pronto!</p>
                </div>
              )}
            </div>
            <Button 
