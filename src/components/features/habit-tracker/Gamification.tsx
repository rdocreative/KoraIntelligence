      ...
      <CardContent className="pt-8 space-y-8 bg-gradient-to-r from-transparent to-white/[0.02]">
        <div className="flex items-center gap-5">
            <div className="h-20 w-20 rounded-[2rem] bg-black/40 flex items-center justify-center text-4xl border border-white/10 shadow-2xl relative group">
                <div className="absolute inset-0 bg-[#4adbc8]/10 blur-2xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
            ...
            <div>
                ...
                <div className="text-xs text-[#4adbc8] font-black uppercase tracking-[0.2em] mt-2 flex items-center gap-2">
        ...
        {nextBadge ? (
          <div className="space-y-4">
             ...
             <div className="h-3 w-full bg-black/40 rounded-full border border-white/5 p-0.5">
                <div 
                    className="h-full bg-gradient-to-r from-[#3bc7b6] to-[#4adbc8] shadow-[0_0_20px_rgba(74,219,200,0.7)] rounded-full transition-all duration-1000" 
                    style={{ width: `${progress}%` }} 
                />
             </div>
             <p className="text-[11px] text-white/30 text-center mt-2 font-black uppercase tracking-[0.15em]">
               Faltam <span className="text-[#4adbc8] font-rajdhani text-base">{nextBadge.threshold - totalPoints} XP</span> para {nextBadge.name}
             </p>
          </div>
        ) : (
