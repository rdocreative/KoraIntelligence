    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-6">
      ...
      <div className="flex gap-2 pb-2 border-b border-[#2a2a35]">
        {['Feed', 'Ranking', 'Grupos'].map((tab, i) => (
          <button 
            key={tab}
            className={`text-xs font-rajdhani font-bold uppercase tracking-wider px-4 py-2 rounded-lg transition-colors ${i === 0 ? 'bg-[#a78bfa]/10 text-[#a78bfa]' : 'text-[#6b6b7a] hover:text-white'}`}
          >
            {tab}
          </button>
        ))}
